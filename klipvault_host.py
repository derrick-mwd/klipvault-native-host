...
_LOG_PATHS = [
    os.path.join(_SCRIPT_DIR, "klipvault_host.log"),
    os.path.join(os.environ.get("TEMP", "/tmp"), "klipvault_host.log"),
    os.path.join(os.getcwd(), "klipvault_host.log"),
]
...
def run_yt_dlp(payload):
    ...
    if cookies:
        if sys.platform == "win32":
            home = _get_windows_home() or os.path.expanduser("~")
            cookies_path = os.path.join(home, ".klipvault_cookies.txt")
        else:
            cookies_path = os.path.join(os.path.expanduser("~"), ".klipvault_cookies.txt")
